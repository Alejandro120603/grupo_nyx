...
const ShaderMaterial = ({
  ...
  const getUniforms = useCallback(() => {
    const preparedUniforms: { [key: string]: { value: number[] | number[][] | number | THREE.Vector3 | THREE.Vector3[] | THREE.Vector2 | THREE.Vector2[]; type?: string } } = {};
    ...
    for (const [uniformName, uniform] of Object.entries(uniforms)) {
      switch (uniform.type) {
        ...
        case "uniform1fv":
          ...
          break;
        case "uniform3fv":
          preparedUniforms[uniformName] = { value: (uniform.value as number[][]).map((v) => new THREE.Vector3().fromArray(v)) };
          break;
        case "uniform2f":
